...
# address: 955 Juniper St Ne, Atlanta, Ga, 30309
# lat: 33.780610
# lon: -84.381980
# coords from https://www.latlong.net/convert-address-to-lat-long.html
# pt <- data.frame(lat = YOURLATHERE, long = YOURLONHERE)
pt <- data.frame(lat = 33.780610, long = -84.381980)
...
geoid <-  c('13121', '13089', '13067', '13135', '13063')

#-------import road files.----------
# files <- list.files(path="./FeatNames", pattern="*.dbf", full.names=TRUE, recursive=FALSE) %>% as.data.frame
files <- list.files(path=here::here("road_shp_features",  "FeatNames"), pattern="*.dbf", full.names=TRUE, recursive=FALSE) %>% as.data.frame
names(files) <- c("path")
# files$GEOID <- substr(files$path, 21, 25)
files$GEOID <- substr(files$path, 78, 82)
...
for (i in 1:nrow(files)) {
  ...
  roads <- read_sf(here::here("road_shp", paste0("tl_2018_", files$GEOID[i], "_roads")))
